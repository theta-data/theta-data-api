...
export class TxService {
  ...
  public async getThetaDataByDate(timezoneOffset: string) {
    ...
    const activeWallets = await this.walletService.getActiveWallet(
      startTimeStamp,
    );
    let obj: {
    ...
    hours.forEach((hourData) => {
      ...
      if (!obj.hasOwnProperty(date)) {
        obj[date] = {
          ...
          theta_fuel_burnt_by_smart_contract:
            hourData.theta_fuel_burnt_by_smart_contract,
        ...
        obj[date].service_payment_transaction +=
          hourData.service_payment_transaction;
        obj[date].split_rule_transaction += hourData.split_rule_transaction;
        obj[date].deposit_stake_transaction +=
          hourData.deposit_stake_transaction;
        obj[date].withdraw_stake_transaction +=
          hourData.withdraw_stake_transaction;
        obj[date].smart_contract_transaction +=
          hourData.smart_contract_transaction;
        obj[date].theta_fuel_burnt += hourData.theta_fuel_burnt;
        obj[date].theta_fuel_burnt_by_smart_contract +=
          hourData.theta_fuel_burnt_by_smart_contract;
        obj[date].theta_fuel_burnt_by_transfers +=
          hourData.theta_fuel_burnt_by_transfers;
      }
    });
  ...
  public async getThetaByHour(timezoneOffset, hours: number = 24 * 7) {
    ...
    const activeWalletList = await this.walletService.getActiveWallet(
      startTime - 3600,
    );
    res.forEach((tx) => {
