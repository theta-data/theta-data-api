...
export class AnalyseService {
  ...
  public async analyseData() {
    ...
    const lastfinalizedHeight = Number(
      (await thetaTsSdk.blockchain.getStatus()).result
        .latest_finalized_block_height,
    );
    height = lastfinalizedHeight - 1000;
    this.logger.debug(
      'analyse Data get latest finalized height from block chain:' + height,
    );
  ...
  async handleOrderCreatedEvent(
    ...
    try {
        ...
        switch (transaction.type) {
            ...
            for (const output of transaction.raw.outputs) {
              ...
              transacitonToBeUpserted.push({
                reward_amount: Number(
                  new BigNumber(output.coins.tfuelwei)
                    .dividedBy('1e18')
                    .toFixed(),
                ),
              ...
              if (transacitonToBeUpserted.length > 900) {
                ...
                this.logger.debug(
                  height +
                    ': stake reward upsert used ' +
                    (moment().unix() - stakeRewardStart),
                );
          ...
          case THETA_TRANSACTION_TYPE_ENUM.send:
            ...
            if (transaction.raw.fee && transaction.raw.fee.tfuelwei != '0') {
              theta_fuel_burnt_by_transfers += new BigNumber(
                transaction.raw.fee.tfuelwei,
              )
                .dividedBy('1e18')
                .toNumber();
            }
          ...
          case THETA_TRANSACTION_TYPE_ENUM.smart_contract:
            ...
            if (transaction.raw.gas_limit && transaction.raw.gas_price) {
              theta_fuel_burnt_by_smart_contract += new BigNumber(
                transaction.raw.gas_price,
              )
      ...
      this.logger.debug(
        'snapshot used time:' + (moment().unix() - startSnapShot),
      );
  ...
  async updateCheckPoint(block: THETA_BLOCK_INTERFACE) {
    try {
      ...
      const [
        eenpTotalNodeNum,
        eenpEffectiveNodeNum,
        eenpTotalTfWei,
        eenpEffectiveTfWei,
      ]: [number, number, BigNumber, BigNumber] = await this.updateEenp(block);
      ...
      if (!res) {
        this.logger.debug(
          'total guardian stake:' +
            parseInt(guTotalThetaWei.dividedBy('1e27').toFixed()),
        );
        try {
          return await this.stakeStatisticsRepository.insert({
            ...
            total_validator_stake_amount: parseInt(
              vaTotalThetaWei.dividedBy('1e18').toFixed(),
            ),
          });
        } catch (e) {
  ...
  async updateValidator(
    ...
    const validatorList = await thetaTsSdk.blockchain.getVcpByHeight(
      block.height,
    );
    if (!validatorList.result || !validatorList.result.BlockHashVcpPairs) {
      throw new Error('no validator BlockHashVcpPairs');
    }
    validatorList.result.BlockHashVcpPairs[0].Vcp.SortedCandidates.forEach(
      (node) => {
        totalNodeNum++;
        node.Stakes.forEach((stake) => {
          ...
          block.hcc.Votes.forEach((vote) => {
            if (vote.ID === node.Holder && !stake.withdrawn) {
              effectiveNodeNum++;
              effectiveThetaWei = effectiveThetaWei.plus(
  ...
  async updateGuardian(
    ...
    for (const guardian of gcpList.result.BlockHashGcpPairs[0].Gcp
      .SortedGuardians) {
      totalNodeNum++;
    ...
    for (let i = 0; i < block.guardian_votes.Multiplies.length; i++) {
      if (block.guardian_votes.Multiplies[i] !== 0) {
        ...
        gcpList.result.BlockHashGcpPairs[0].Gcp.SortedGuardians[
          i
  ...
  async updateEenp(
    ...
    eenpList.result.BlockHashEenpPairs[0].EENs.forEach((eenp) => {
      ...
      block.elite_edge_node_votes.Multiplies.forEach((value, index) => {
        if (
          block.elite_edge_node_votes.Addresses[index] == eenp.Holder &&
          value !== 0
        ) {
      ...
      eenp.Stakes.forEach((stake) => {
        ...
        if (isEffectiveNode && !stake.withdrawn) {
          effectiveTfuelWei = effectiveTfuelWei.plus(
            new BigNumber(stake.amount),
          );
        }
      });
