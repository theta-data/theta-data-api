...
class SendTx extends Tx {
  constructor(senderAddr, outputs, feeInTFuelWei, senderSequence) {
    ...
    let feeInTFuelWeiBN = BigNumber.isBigNumber(feeInTFuelWei)
      ? feeInTFuelWei
      : new BigNumber(feeInTFuelWei)
    ...
    let txInput = new TxInput(
      senderAddr,
      totalThetaWeiBN,
      totalTfuelWeiBN.plus(feeInTFuelWeiBN),
      senderSequence
    )
    this.inputs = [txInput]
...
class DepositStakeTx extends StakeTx {
  constructor(source, holderAddress, stakeInThetaWei, feeInTFuelWei, purpose, senderSequence) {
    ...
    let stakeInThetaWeiBN = BigNumber.isBigNumber(stakeInThetaWei)
      ? stakeInThetaWei
      : new BigNumber(stakeInThetaWei)
